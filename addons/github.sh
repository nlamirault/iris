...
docker build -t iris/release .

echo -e "\033[32;01m[$APP] Make binaries \033[0m"
docker run --rm \
       -v `pwd`:/tmp/ \
       iris/release \
       gox "${OS_PLATFORM_ARG[@]}" "${OS_ARCH_ARG[@]}" -output="/tmp/iris_{{.OS}}-{{.Arch}}"

echo -e "\033[32;01m[$APP] Make release \033[0m"
docker run --rm -e GITHUB_TOKEN iris/release \
...
for BINARY in iris_*; do
    docker run --rm -e GITHUB_TOKEN -v `pwd`:/go/src/github.com/nlamirault/iris \
       iris/release github-release upload \
       --user $USERNAME \
       --repo $REPO \
       --tag $VERSION \
       --name $BINARY \
